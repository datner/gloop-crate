...
export const GetLitmusVersions = GetGithubVersions('Litmus', 'litmuschaos', 'litmus');

export const Litmus = (org: string, distro: Distro, timestamp: number, version: O.Option<string> = O.some('latest')) =>
  F.pipe(
    GetLitmusVersions,
              ...
              Cont.Step({
                ...
                commands: [
                  Cont.Command({
                    ...
                    run: F.pipe(
                      CloneRepoChunk({ name: 'Litmus', git: 'https://github.com/litmuschaos/litmus', version: `v${version}` }),
                      A.appendAll([`cd /builder/src/litmus`, 'make', `mv bin/litmus /builder/litmus/bin`])
