...
export const GetCiliumVersions = GetGithubVersions('Cilium', 'cilium', 'cilium');

export const Cilium = (org: string, distro: Distro, timestamp: number, version: O.Option<string> = O.some('latest')) =>
  F.pipe(
    GetCiliumVersions,
              ...
              Cont.Step({
                ...
                commands: [
                  Cont.Command({
                    ...
                    run: F.pipe(
                      CloneRepoChunk({ name: 'cilium', git: 'https://github.com/cilium/cilium', version: `v${version}` }),
                      A.appendAll([`cd /builder/src/cilium`, 'make', `mv bin/cilium /builder/cilium/bin`])
