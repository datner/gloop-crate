...
export const GetTektonCDTriggersVersions = GetGithubVersions('TektonCD Triggers', 'tektoncd', 'triggers');

export const TektonCDTriggers = (org: string, distro: Distro, timestamp: number, version: O.Option<string> = O.some('latest')) =>
  F.pipe(
    GetTektonCDTriggersVersions,
              ...
              Cont.Step({
                ...
                commands: [
                  Cont.Command({
                    ...
                    run: F.pipe(
                      CloneRepoChunk({ name: 'tektoncd-triggers', git: 'https://github.com/tektoncd/triggers', version: `v${version}` }),
                      A.appendAll([`cd /builder/src/tektoncd-triggers`, `mv bin/tektoncd-pipeline /builder/tektoncd-triggers/bin`])
