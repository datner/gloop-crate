...
export const GetArgoEventsVersions = GetGithubVersions('Argo Events', 'argoproj', 'argo-events');

export const ArgoEvents = (org: string, distro: Distro, timestamp: number, version: O.Option<string> = O.some('latest')) =>
  F.pipe(
    GetArgoEventsVersions,
              ...
              Cont.Step({
                ...
                commands: [
                  Cont.Command({
                    ...
                    run: F.pipe(
                      CloneRepoChunk({ name: 'argoevents', git: 'https://github.com/argoproj/argo-events', version: `v${version}` }),
                      A.appendAll([`cd /builder/src/argoevents`, `mv bin/argoevents /builder/argoevents/bin`])
