...
export const GetRedisVersions = GetGithubVersions('Redis', 'redis', 'redis');

export const Redis = (org: string, distro: Distro, timestamp: number, version: O.Option<string> = O.some('latest')) =>
  F.pipe(
    GetRedisVersions,
              ...
              Cont.Step({
                ...
                commands: [
                  Cont.Command({
                    ...
                    run: F.pipe(
                      CloneRepoChunk({ name: 'redis', git: 'https://github.com/redis/redis', version: `v${version}` }),
                      A.appendAll([
                        `cd /builder/src/redis`,
                        'make distclean',
                        'make BUILD_TLS=yes',
