...
export const GetScyllaOperatorVersions = GetGithubVersions('ScyllaDB Operator', 'scylladb', 'scylla-operator');

export const ScyllaOperator = (org: string, distro: Distro, timestamp: number, version: O.Option<string> = O.some('latest')) =>
  F.pipe(
    GetScyllaOperatorVersions,
              ...
              Cont.Step({
                ...
                commands: [
                  Cont.Command({
                    ...
                    run: F.pipe(
                      CloneRepoChunk({ name: 'scylladb', git: 'https://github.com/scylladb/scylla-operator', version: `v${version}` }),
                      A.appendAll([
