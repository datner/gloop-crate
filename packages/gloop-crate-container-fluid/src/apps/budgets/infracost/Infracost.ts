...
export const GetInfraCostVersions = GetGithubVersions('Infracost', 'infracost', 'infracost');

export const Infracost = (org: string, distro: Distro, timestamp: number, version: O.Option<string> = O.some('latest')) =>
  F.pipe(
    GetInfraCostVersions,
              ...
              Cont.Step({
                ...
                commands: [
                  Cont.Command({
                    ...
                    run: F.pipe(
                      CloneRepoChunk({ name: 'infracost', git: 'https://github.com/infracost/infracost', version: `v${version}` }),
                      A.appendAll([`cd /builder/src/infracost`, 'make', `mv bin/velero /builder/velero/bin`])
