...
export const GetOpenCostVersions = GetGithubVersions('Opencost', 'opencost', 'opencost');

export const OpenCost = (org: string, distro: Distro, timestamp: number, version: O.Option<string> = O.some('latest')) =>
  F.pipe(
    GetOpenCostVersions,
              ...
              Cont.Step({
                ...
                commands: [
                  Cont.Command({
                    ...
                    run: F.pipe(
                      CloneRepoChunk({ name: 'opencost', git: 'https://github.com/opencost/opencost', version: `v${version}` }),
                      A.appendAll([`cd /builder/src/opencost`, 'make', `mv bin/velero /builder/opencost/bin`])
