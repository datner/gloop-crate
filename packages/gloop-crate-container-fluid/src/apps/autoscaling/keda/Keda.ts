...
export const GetKedaVersions = GetGithubVersions('Keda', 'kedacore', 'keda');

const kedaPackages = HashMap.fromIterable<Distro, A.NonEmptyArray<string>>([
  // ['Alpine', ['go']],
  // ['Debian', ['go']],
  // ['Photon', ['go']],
  // ['Oracle', ['go']],
...
export const Keda = (org: string, distro: Distro, timestamp: number, version: O.Option<string> = O.some('latest')) =>
  F.pipe(
    GetKedaVersions,
              ...
              Cont.Step({
                ...
                commands: [
                  Cont.Command({
                    ...
                    packages: F.pipe(kedaPackages, HashMap.get(distro), O.getOrNull) as A.NonEmptyArray<string>,
                    run: F.pipe(
                      CloneRepoChunk({ name: 'keda', git: 'https://github.com/kedacore/keda', version: `v${version}` }),
                      A.appendAll([
                        `cd /builder/src/keda`,
